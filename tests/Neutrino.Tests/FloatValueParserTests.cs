...
public class FloatValueParserTests
{
    ...
    [Fact]
    public void Float_Format_WithWholeNumber_ReturnsStringRepresentation()
    {
        var parser = Float();
        var result = parser.Format(42.0f);
        Assert.Equal("42", result);
    }

    [Fact]
    public void Float_Format_WithDecimalNumber_ReturnsStringRepresentation()
    {
        var parser = Float();
        var result = parser.Format(42.5f);
        Assert.Equal("42.5", result);
    }

    [Fact]
    public void Float_Format_WithNegativeNumber_ReturnsStringRepresentation()
    {
        var parser = Float();
        var result = parser.Format(-123.45f);
        Assert.Equal("-123.45", result);
    }

    [Fact]
    public void Float_Format_WithZero_ReturnsZero()
    {
        var parser = Float();
        var result = parser.Format(0.0f);
        Assert.Equal("0", result);
    }

    [Fact]
    public void Float_Format_WithVerySmallNumber_ReturnsScientificNotation()
    {
        var parser = Float();
        var result = parser.Format(0.0000001f);
        Assert.Contains("E", result);
    }

    [Fact]
    public void Float_Format_WithVeryLargeNumber_ReturnsScientificNotation()
    {
        var parser = Float();
        var result = parser.Format(123456789012345f);
        Assert.Contains("E", result);
    }
}
